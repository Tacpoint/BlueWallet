...
const WalletsAddTaproot = () => {
  ...
  const onLetsStartPress = () => {
    navigate('WalletsAddTaprootStep2', { m, n, format, walletLabel });
  };
  ...
  const getCurrentlySelectedFormat = code => {
    switch (code) {
      case 'format':
        return WalletsAddTaproot.getCurrentFormatReadable(format);
      case 'quorum':
    ...
    <SafeAreaView style={stylesHook.root}>
      <View style={styles.descriptionContainer}>
        <View style={styles.imageWrapper}>
          <LottieView source={require('../../img/bitcoin_animation.json')} autoPlay ref={loadingAnimation} loop={false} />
        </View>
        <BlueSpacing20 />
        <Text style={[styles.textdesc, stylesHook.textdesc]}>
          {loc.taproot.what_is_taproot}
        </Text>
...
WalletsAddTaproot.getCurrentFormatReadable = f => {
  switch (f) {
...
WalletsAddTaproot.navigationOptions = navigationStyle({
  headerTitle: null,
});

export default WalletsAddTaproot;
